...
- Edges use negative `env(safe-area-inset-*)` values (top/right/bottom/left) instead of `0`, paired with `viewport-fit=cover` on the viewport meta tag. This pushes the fixed-positioned ivy past the iOS layout-viewport boundaries into the status bar, home-indicator, and landscape side safe-area zones, so the texture reaches the device edges. On desktop and Android the env() values are `0` and the math collapses — only iOS is affected. The `.container` deliberately doesn't use these insets so body text stays out of the notch. The companion `apple-mobile-web-app-status-bar-style` meta tag is set to `default`, not `black-translucent`: counterintuitively, "default" produces the genuine see-through behavior (whatever the page paints behind the status bar shows through), while "black-translucent" paints a translucent wash that hid the ivy in portrait standalone mode.
